...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    temp1=torch.flatten(x_train[i]).clone()
    for j in range(num_test):
      temp2=torch.flatten(x_test[j]).clone()
      dists[i][j]=(temp1-temp2).pow(2).sum().sqrt()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  test=x_test.clone()
  for i in range(num_train):
    temp=x_train[i].clone()
    dists[i:,]=(test-temp).reshape((num_test,-1)).pow(2).sum(axis=1).sqrt()
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  ##temp1 = x_train.clone().reshape(num_train, 1, -1)
  ##test_flat = x_test.clone().reshape(num_test, -1)
  ##dists = torch.sqrt(torch.sum(torch.pow(temp1 - test_flat, 2), axis=2))
  #DOUBT : the above code took more time than 1 looped version,why?
  
  temp1 = x_train.reshape(num_train,-1)
  temp2 = x_test.reshape(num_test,-1)
  dists = torch.sqrt( torch.sum(temp1**2,dim = 1,keepdims = True) + torch.sum(temp2.t()**2,dim = 0,keepdims = True) -2*torch.mm(temp1,temp2.t()) )
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
